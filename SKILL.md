...
6. **浏览器五板块 = 优先数据池（均可优先调用）**：极速模式优先从「手册浏览器」五个内置板块的打包数据取数（离线即用、随 skill 分发），按问题类型对应调用：
   - 快捷键 → 快捷键 tab（`shortcuts.md`）
   - 常用操作（怎么XXX）→ 高频速查 tab（42 条，`高频速查.md`/`quickref.json`）
   - 功能 / 操作 / 某面板在哪 → 全文检索 tab（可切中·英，即 `21.1原文检索.txt` / `21.1英文原文检索.txt`）
   - 插件中英名 / 是否 Studio 专属 → 插件中英对照 tab（`Resolve_FX中英对照.md` / `Fairlight音频插件.md`）
   - 脚本 / API / MCP / 插件开发 → 官方开发文档 tab（`官方开发文档蒸馏.md`）
   **能用内置板块数据答的，绝不外求或重造**；只有五个板块都未覆盖才明说"手册未覆盖"并联网补充。注：「优先调用」指**调取这些数据来组答案**，实际弹出 HTML 浏览器仍走「答后必问」，用户确认后才 `present_files`。
