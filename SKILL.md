...
2. **检索最多两轮**：先 `Grep` 命中最可能的 1-2 个文件。**简单操作问询（怎么变速 / 怎么重置参数 / 某功能在哪 / 怎么XXX）走快路径**：① `references/高频速查.md` 或 `references/quickref.json`（42 条聚焦常用操作，**命中即答、绝不碰大文件**，最快）；② 未命中才 `references/21.1原文检索.txt`（4351 页全文）；③ 仅当汉化歧义/漏译才 `references/21.1英文原文检索.txt`。其余类型：快捷键→`shortcuts.md`；插件名→两个中英对照 md；开发/API/MCP→`官方开发文档蒸馏.md`。命中即答，**不再扩范围精读**。查不到就明说"手册未覆盖"，不反复翻文件；确需补充时可联网搜索（官方社区 / B站·YouTube 博主）后再答。
...
6. **浏览器五板块 = 优先数据池（命中优先 / 未命中降级 / 开发优先）**：极速模式优先从「手册浏览器」五个内置板块的打包数据取数（离线即用、随 skill 分发）。路由逻辑：
   - **关键词命中优先**（问快捷键 / 怎么操作某功能 / 插件中文名等，且在结构化数据里能 `Grep` 到）→ 直接调对应板块：
     ...
     - 常用操作 / 普通操作（怎么XXX，如怎么变速、怎么重置参数）→ 高频速查 tab（42 条，`高频速查.md`/`quickref.json`）
